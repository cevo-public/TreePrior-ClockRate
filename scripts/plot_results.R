...
load_data = function(filename) {
  ...
  if (n <= 10001 ) {
    ...
    data = data[as.integer(0.1*n+1):n,c("clockRate", "treeHeight", "treeLength", "totalEvolution")]
    
    # Post-processing  
    colnames(data) = c('Clock rate', 'Tree height', 'Tree length', 'Total divergence')
...
create_plot = function(data_name) {
  ...
  ggsave(paste('../figures/', data_name, '.pdf', sep = ''), width = 8.5, height = 12, units = 'cm')
}
