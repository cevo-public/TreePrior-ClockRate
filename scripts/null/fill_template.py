...
def fill_template(output_dir, seq_length, rate, treenum, num_runs = 1):
	tree_f = open('tree_'+str(treenum)+'_fixed.nwk')
	tree = tree_f.read()
	tree_f.close()

	taxa_f = open('taxa.txt')
	taxa = taxa_f.read()
	taxa_f.close()
	...
	hook_dict['OUTPUT_HOOK'] = output_dir + "ebola_" + str(seq_length) + "_" + str(rate) + "_" + str(treenum)
