from fill_template_fixed import fill_template
...
def fill_analysis_template(output_dir, template_filename, seq_length, rate, popsize, chain_length):
	hook_dict = {}
	...
	hook_dict['CLOCKRATE_HOOK'] = rate
	hook_dict['POPSIZE_HOOK'] = popsize
...
def make_prior_analysis(analysis_filename, output_filename):
	...
	replaced = original.replace(analysis_name, output_name)

	out_f = open(analysis_filename, 'w')
	out_f.write(replaced)
	out_f.close()

	shutil.move(analysis_filename, output_filename)
...
def main():	
	...
	for seq_length in seq_lengths:
		for rate in rates:
			...
			fill_template(output_dir, seq_length, rate, num_runs)
			# adjust template for ANALYSIS
			fill_analysis_template(output_dir, args.template_filename, seq_length, rate, popsize, chain_length[seq_length])
	...
	for rate in rates:
		for i in xrange(num_runs):
			...
			make_prior_analysis(analysis_filename, output_dir + 'ebola_0_' + str(rate) + "_" + str(i) + ".xml")
