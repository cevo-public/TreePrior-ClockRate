...
load_data = function(filename, colless, sackin) {
  ...
  if (n > 10001 || n != m) {
    ...
    data = data[as.integer(0.1*n+1):n,c("clockRate", "treeHeight", "treeLength", "posterior","totalEvolution", "colless", "sackin", "gamstat")]
    
    # Post-processing  
    colnames(data) = c('Clock rate', 'Tree height', 'Tree length', 'Posterior', 'Total divergence', "Colless", "Sackin", "Gamma statistic")
...
plot_results <- function(results_dir="../results/", output_dir="../results/", nruns = 10, seq_lengths = c(0, 100, 500, 1000, 15000), rates = c(0.1)) {
    ...
    dummy_df$colless = 2696
    dummy_df$sackin = 3820
    dummy_df$gamstat = -16.898719
    colnames(dummy_df) = c("seq_length", "rate", "run", "setup", 'Clock rate', 'Tree height', 'Tree length', 'Total divergence', 'Colless', 'Sackin', 'Gamma statistic')
    ...
    ggsave(paste(output_dir, "simulation_summary_treestats_",rates[1],".pdf", sep = ""), width = 8.5, height = 17.5, units = "cm")
    ...
    ggplot(aes(x = seq_length, y = value, group = run), data = subset(df, variable != 'Posterior' & seq_length != 1)) +
      ...
      geom_hline(data = dummy_df, aes(yintercept = value), linetype = 3) +
      theme_bw(base_size = 10)
    ggsave(paste(output_dir, "simulation_summary_prior_treestats_",rates[1],".pdf", sep = ""), width = 8.5, height = 17.5, units = "cm")
    ...
    ggplot(aes(x = seq_length, y = value, group = run), data = subset(data_stats, statistic == "Coverage")) +
      ...
      theme_bw(base_size = 10)
    ggsave(paste(output_dir, "simulation_coverage_treestats_",rates[1],".pdf", sep = ""), width = 8.5, height = 17.5, units = "cm")
    ...
    ggplot(aes(x = seq_length, y = value), data = subset(data_stats, statistic == "Relative HPD width" & seq_length != 0)) +
      ...
      geom_hline(yintercept=0, linetype = 3) +
      theme_bw(base_size = 10)
    ggsave(paste(output_dir, "simulation_relhpdwidth_treestats_",rates[1],".pdf", sep = ""), width = 8.5, height = 17.5, units = "cm")
    ...
    ggplot(aes(x = seq_length, y = value), data = subset(data_stats, statistic == "Relative bias" & seq_length != 0)) +
      ...
      geom_hline(yintercept=0, linetype = 3) +
      theme_bw(base_size = 10)
    ggsave(paste(output_dir, "simulation_relbias_treestats_",rates[1],".pdf", sep = ""), width = 8.5, height = 17.5, units = "cm")
    ...
    ggplot(aes(x = seq_length, y = value), data = subset(data_stats, statistic == "RMSD" & seq_length != 0)) +
      ...
      geom_hline(yintercept=0, linetype = 3) +
      theme_bw(base_size = 10)
    ggsave(paste(output_dir, "simulation_RMSD_treestats_",rates[1],".pdf", sep = ""), width = 8.5, height = 17.5, units = "cm")
    ...
    ggplot(aes(x = seq_length, y = value), data = subset(data_stats, statistic == "Relative bias")) +
      ...
      geom_hline(yintercept=0, linetype = 3) +
      theme_bw(base_size = 10)
    ggsave(paste(output_dir, "simulation_relbias_prior_treestats_",rates[1],".pdf", sep = ""), width = 8.5, height = 17.5, units = "cm")
