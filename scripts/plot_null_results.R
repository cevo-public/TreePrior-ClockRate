...
setups = c("const_coal")
results_dir = "../results/results_simulation/"
null_results_dir = "../results/results_null/"
null_trees_dir = "../results/results_nullfixed/"
output_dir = "../figures/null/"
data_pre = "ebola_"
nruns = 10
seq_lengths = c(0, 100, 500, 1000, 15000)
rates = 0.1
numtrees = 9
null_trees_stats = read.table(paste0(null_trees_dir,"ebola_0_0.1_0.downsampled10.treestat"), header=TRUE, sep="\t")

for (tree in 0:(numtrees-1)) {
  # load data, always only used replicate 0 for each tree
  df = data.frame()
  for (s in setups) {
    for (i in 0:(nruns-1)) {
      for (l in seq_lengths) {
        for (r in rates) {
          if (l == 0)
            ...
            filename = paste(null_results_dir, data_pre, l, "_", r, "_",tree,"_", i, sep = "")
  ...
  dummy_df$treeHeight = null_trees_stats$Tree.height[tree+1]
  dummy_df$treeLength = null_trees_stats$Tree.length[tree+1]
  dummy_df$clockRate = dummy_df$rate
  dummy_df$totalEvolution = dummy_df$clockRate * dummy_df$treeLength
  colnames(dummy_df) = c("seq_length", "rate", "run", "setup", 'Tree height', 'Tree length', 'Clock rate', 'Total divergence')
  dummy_df = melt(dummy_df, id.vars = 1:4)
  
  range_clockrate  <- data.frame(seq_length = seq_lengths[1], run=1, value = c(0.06,0.14), variable='Clock rate')
  ...
  ggsave(paste(output_dir, "null_simulation_summary_",tree,".pdf", sep = ""), width = 8.5, height = 10, units = "cm")
  ...
  ggplot(aes(x = seq_length, y = value, group = run), data = subset(df, variable != 'Posterior' & seq_length != 1)) +
    ...
    geom_hline(data = dummy_df, aes(yintercept = value), linetype = 3) +
    theme_bw(base_size = 10)
  ggsave(paste(output_dir, "null_simulation_summary_prior_",tree,".pdf", sep = ""), width = 8.5, height = 10, units = "cm")
  ...
  ggplot(aes(x = seq_length, y = value, group = run), data = subset(data_stats, statistic == "Coverage")) +
    ...
    theme_bw(base_size = 10)
  ggsave(paste(output_dir, "null_simulation_coverage_",tree,".pdf", sep = ""), width = 8.5, height = 10, units = "cm")
  ...
  ggplot(aes(x = seq_length, y = value), data = subset(data_stats, statistic == "Relative HPD width" & seq_length != 0)) +
    ...
    geom_hline(yintercept=0, linetype = 3) +
    theme_bw(base_size = 10)
  ggsave(paste(output_dir, "null_simulation_relhpdwidth_",tree,".pdf", sep = ""), width = 8.5, height = 10, units = "cm")
  ...
  ggplot(aes(x = seq_length, y = value), data = subset(data_stats, statistic == "Relative bias" & seq_length != 0)) +
    ...
    geom_hline(yintercept=0, linetype = 3) +
    theme_bw(base_size = 10)
  ggsave(paste(output_dir, "null_simulation_relbias_",tree,".pdf", sep = ""), width = 8.5, height = 10, units = "cm")
